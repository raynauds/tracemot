...
|---|---|---|---|
| `proposeLevel` | `{ modeId, levelId }` | ni `phase === "map"` ni `won` ; proposition déjà ouverte ; `cellState(union, id)` ∉ {active, validated} (le **rejeu d'un niveau validé est permis**, → Q17 — même prédicat « jouable » que la carte) | crée `proposal` kind "level" (auto-accepté par le proposeur, `openedAt = Rune.gameTime()`) ; seul en room → lancement immédiat |
